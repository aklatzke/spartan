		...
		function widget( $args, $instance ) {
			...
			if($query->have_posts()){
				...
				while($query->have_posts()){ $query->the_post();
					...
					echo '<div class="recent-post-widget-title"><h3><a href="' . get_permalink() . '" >' . get_the_title() . '</a></h3></div>';
					echo '<div class="blog-author">' . gdlr_get_blog_info(array('author'), false) . '</div>';
					echo '<div class="recent-post-widget-info">' . gdlr_get_blog_info(array('date'), false) . '</div>';
					echo '</div>';
					$thumbnail = gdlr_get_image(get_post_thumbnail_id(), 'blog-teaser-349x150');
					if( !empty($thumbnail) ){
					...
					echo '</div>';
					//echo '<pre>' .  var_dump($thumbnail) . '</pre>';
				}
				echo '<div class="clear"></div>';
				echo '</div>';
				echo '<!-- /gdlr-recent-post-widget -->';
...
// 			echo '<pre>' .  var_dump($after_widget) . '</pre>';
		}
